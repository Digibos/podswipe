...
export default class CircularSlider extends PureComponent {
  ...
  componentWillMount() {
    this._sleepPanResponder = PanResponder.create({
      ...
      onPanResponderMove: (evt, {moveX, moveY}) => {
        ...
        clearTimeout(this.mooveTimeout_);
        this.mooveTimeout_ = setTimeout(() => {
          this.setState({moove: false});
        }, 1000)
      },
    });
  ...
  render() {
    const {
      startAngle, angleLength, segments, strokeWidth, radius, gradientColorFrom, gradientColorTo, bgCircleColor,
      showClockFace, clockFaceColor, startIcon, stopIcon
    } = this.props;
        ...
        >
          <Defs>
            {
              range(segments).map(i => {
                ...
                return (
                  <LinearGradient key={i} id={getGradientId(i)} x1={fromX.toFixed(2)} y1={fromY.toFixed(2)}
          ...
          <G transform={{translate: `${strokeWidth / 2 + radius + 1}, ${strokeWidth / 2 + radius + 1}`}}>
            ...
            <G
              ...
              onPressIn={() => this.setState({moove: true, angleLength: angleLength + Math.PI / 2})}
              {...this._wakePanResponder.panHandlers}
            >
              <Circle
                r={(strokeWidth - 1) / (this.state.moove ? 1.2 : 2)}
                fill={gradientColorTo}
                stroke={bgCircleColor}
                strokeWidth="2"
              />
